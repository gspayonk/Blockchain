...
def mine():
    ...
    try:
        ...
        print("ERROR: Must be JSON Format.")
        return "Error"
    ...
    for key in validate:
        if key not in req:
            response = {
                'Error': 'Invalid format, Please provide { proof: & id: }'
            }
            code = 400
        ...
        chain_id = req['id']
        guess_proof = req['proof']

        if blockchain.valid_proof(block, guess_proof):

            blockchain.new_transaction('0', req['id'], 1)
            previous = blockchain.hash(blockchain.last_block)
            created = blockchain.new_block(guess_proof, previous)
